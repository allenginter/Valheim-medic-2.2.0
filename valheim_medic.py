...
class UnityScannerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Valheim Mod Medic v2.0")
        ...
        tk.Checkbutton(f_frame, text="Scan for Missing/Standard Shaders (Pink Risk)", variable=self.check_shaders).pack(anchor="w")
    ...
    def scan_thread(self, scan_path):
        ...
        bad_shaders = [b"Standard", b"Hidden/InternalErrorShader", b"Standard (Specular setup)"]
        ...
        for i, file_path in enumerate(files_to_check):
            ...
            # Get path relative to the selected root (e.g., 'ModName/Folder/file.png')
            rel_path = os.path.relpath(file_path, scan_path)
            # Identify the top-most folder as the Mod ID
            mod_folder = rel_path.split(os.sep)[0]

            # 1. Texture Dimension Check
            if self.check_dim.get() and ext in [".png", ".jpg", ".dds"]:
                try:
            ...
            # 2. Shader Content Search
            if self.check_shaders.get() and ext in [".unity3d", ".bundle", ".assets"]:
                try:
                    with open(file_path, "rb") as f:
                        content = f.read(50000)
                        for shader in bad_shaders:
                            if shader in content:
                                self.root.after(0, self.update_log, f"[ALERT] [{mod_folder}] Incompatible Shader '{shader.decode()}' in {rel_path}")
                except: pass
            ...
            if i % 5 == 0: self.root.after(0, lambda v=i: self.progress.config(value=v))
    ...
    def finish_ui(self):
        ...
        messagebox.showinfo("Scan Complete", "Diagnosis finished. Check the log for flagged issues.")

if __name__ == "__main__":
    root = tk.Tk()
    UnityScannerGUI(root)
    root.mainloop()