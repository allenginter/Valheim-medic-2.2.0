...
# This ensures the config file is always saved next to your .py script
...
class UnityScannerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Valheim Mod Medic v2.3 - Aggressive Mode")
        ...
        # Load the last path immediately on startup
        self.load_config()
        ...
        tk.Checkbutton(f_frame, text="Aggressive Shader Scan (Check Every File)", variable=self.check_shaders).pack(anchor="w")
    ...
    def browse(self):
        # Open the browser at the last known path if it exists
    ...
    def scan_thread(self, scan_path):
        ...
        for i, file_path in enumerate(files_to_check):
            ...
            # 2. Aggressive Shader Search (No extension filter)
            if self.check_shaders.get():
                try:
                    # Skip very large files (>200MB) to prevent freezing
    ...
    def finish_ui(self):
        ...
        messagebox.showinfo("Scan Complete", "Aggressive diagnosis finished.")
