...
class UnityScannerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Valheim Mod Medic v2.6.1 - Binary Signature Scan")
        ...
        tk.Checkbutton(f_frame, text="Binary Signature Scan (Exclude Audio/DLL)", variable=self.check_shaders).pack(anchor="w")
    ...
    def scan_thread(self, scan_path):
        ...
        # Specific signatures found in modern Unity assets that break Valheim
        bad_signatures = [
            b"URP", 
            b"ShaderGraph",
            b"DOTS",
        ...
        # Files that often contain the signatures 'SRP' or 'URP' as part of their data
        exclude_extensions = [
            ".dll", ".json", ".txt", ".md", ".xml", ".pdb", 
            ".ogg", ".wav", ".mp3", ".png", ".jpg", ".jpeg", ".dds"
        ]
        ...
        for i, file_path in enumerate(files_to_check):
            ...
            # 2. Binary Signature Scan (Filtering for Asset Data only)
            if self.check_shaders.get() and ext not in exclude_extensions:
                try:
